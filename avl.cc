...
void balance( Node *& x ) {
  // ******** TO BE COMPLETED ********
    ...
    if (height(x->left)-height(x->right)>1){
		if (height(x->left)-height(x->left->left)==1) rotateRight(x);
		else if (height(x->left)-height(x->left->right)==1)  doubleRotateRight(x);
	}
	else if (height(x->right)-height(x->left)>1){
		if (height(x->right)-height(x->right->right)==1) rotateLeft(x);
		else if (height(x->right)-height(x->right->left)==1)  doubleRotateLeft(x);
	}
}
